...
public class SerialPrinter implements Runnable, Printer {
	...
	private void printAndWaitQueue() throws InterruptedException {
		...
		while (state.connected && !state.reset) {
			if (mConn.getType() == PrinterConnection.BLUETOOTH)
				Thread.sleep(5); //Response will take at least 5 ms, safe CPU cylces
...
//		cons.log("TIME","BGTTime:"+bufferedGCodetime);
	...
	private void onFinish() {
		if (state.streaming)
			addToPrintQueue(GCodeFactory.getGCode("M29", -29), true); //Stop sd streaming
		...
		if (state.printspeed != 100)
			addToPrintQueue(GCodeFactory.getGCode("M220 100", -220), true); //set speed back to 100%
		if (state.extrfactor != 100)
			addToPrintQueue(GCodeFactory.getGCode("M221 100", -221), true); //set extr back to 100%
	}
