...
public class GCodeXYEFMinBFB extends GCodeAbstractNoData {
	...
	public GCodeXYEFMinBFB(String line, GCDEF code) {
		super(code);
	}
	...
	public float getExtrusion() {
		//	if(!isInitialized(Constants.E_MASK)) return 0; //wrong for bfb
		return extrusion;
	}
	 ...
	 * bfb has extrusion only (no e_mask)
	 *
	 * @return true is extruding
	 */
	@Override
	...
	public int getCodeline(byte[] buffer) {
		...
		if (isInitialized(Constants.E_MASK)) { //bfb might not
			buffer[len++] = Constants.spaceb;
			buffer[len++] = Constants.Eb;
			len = Constants.floatToString5(e, buffer, len);
		}

		if (isInitialized(Constants.F_MASK)) { //bfb might not
			buffer[len++] = Constants.spaceb;
